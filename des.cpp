...
void DES_Efun(char input[8], char key_in[8], int output[64])
{
	...
	CharToBit(key_in, key, 8);//CHAR转换BIT
	subKey_fun(key, subkeys);//子密钥 
	for (int i = 0; i < 32; i++)
	{
		l[0][i] = Ip[i];
		r[0][i] = Ip[32 + i];
...
void DES_Dfun(int input[64], char key_in[8], char output[8])
{
	...
	CharToBit(key_in, key, 8);//CHAR转换BIT正确！  
	subKey_fun(key, subkeys);//子密钥逆推正确！  
	for (int i = 0; i < 32; i++)
	{
		l[0][i] = Ip[i];
		r[0][i] = Ip[32 + i];
